...
const logger = (0, pino_1.default)({ name: 'jira-ticket-service' });
...
class TicketService {
     ...
     */
    async fetchTicketComments(ticketKey) {
        try {
        ...
        const parseNode = (node) => {
            ...
            switch (node.type) {
                ...
                case 'listItem':
                    ...
                    return `${bullet} ${(node.content || []).map((child) => parseNode(child)).join('')}`;
                case 'codeBlock':
                    return `\`\`\`\n${node.content?.[0]?.text || ''}\n\`\`\`\n\n`;
                case 'blockquote':
                    return `> ${(node.content || []).map((child) => parseNode(child)).join('')}`;
     ...
     */
    async checkProjectAccess(projectKey) {
        try {
            const projects = await this.client.getProjects();
            return projects.values.some((project) => project.key === projectKey);
        }
        catch (error) {
