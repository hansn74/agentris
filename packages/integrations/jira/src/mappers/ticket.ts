 ...
 */
export function mapJiraTicketToInternal(
  jiraTicket: JiraTicket,
  ticketService?: TicketService
): InternalTicket {
  // Map Jira status to internal status
  const mapStatus = (jiraStatus: string): InternalTicket['status'] => {
 ...
 */
function parseSimpleADF(doc: any): string {
 ...
 */
export function mapStatusToTransition(
  status: InternalTicket['status'],
  availableTransitions: Array<{ id: string; name: string }>
): string | null {
  const transitionMap: Record<InternalTicket['status'], string[]> = {
    NEW: ['open', 'create', 'new'],
