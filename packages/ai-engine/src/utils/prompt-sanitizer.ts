...
export class PromptSanitizer {
  ...
  static sanitizeFieldRequirement(requirement: any): any {
    const schema = z.object({
      fieldName: z.string().max(MAX_LENGTHS.fieldName).regex(/^[a-zA-Z][a-zA-Z0-9_]*$/),
      fieldLabel: z.string().max(MAX_LENGTHS.fieldName),
      description: z.string().max(MAX_LENGTHS.generalText),
      formula: z.string().max(MAX_LENGTHS.formula).optional(),
      picklistValues: z.array(z.string().max(255)).max(500).optional(),
    }).passthrough();
    ...
    try {
      ...
      if (validated.picklistValues) {
        validated.picklistValues = validated.picklistValues.map(v => 
          this.normalizeText(v).slice(0, 255)
        );
      }
      ...
      if (error instanceof z.ZodError) {
        throw new Error(`Invalid field requirement: ${error.issues.map((e: any) => e.message).join(', ')}`);
      }
      throw error;
    }
  }
   ...
   */
  static sanitizeText(input: string, maxLength: number = MAX_LENGTHS.generalText): string {
    if (!input || typeof input !== 'string') {
      return '';
    }
   ...
   */
  private static checkForInjectionPatterns(input: string): string {
...
export const _testPatterns = INJECTION_PATTERNS;